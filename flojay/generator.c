...
static
int fj_generate(struct fj_generator * self, PyObject * obj)
{
    char * buf;
    Py_ssize_t len;
    ...
    if( obj == Py_None ) {
        ...
        PyObject * newobj =
            PyObject_CallFunctionObjArgs(self->encoder->default_func,
                                         obj, NULL);
        ...
        int return_value = fj_generate(self, newobj);
                  ...
                  PyObject * args)
{
    ...
    yajl_gen_config(self->gen,
                    yajl_gen_beautify,
                    (self->encoder->beautify == Py_True) ? true : false);
...
fj_generator_next(struct fj_generator * self)
{
    ...
    while( true ) {
        if( (NULL == self->stack)
           || (self->stack->type == STACK_TYPE_UNKNOWN) ) {
            status = yajl_gen_get_buf(self->gen,
                                      (const unsigned char **) &buf,
                                      &len);
            if( fj_handle_yajl_error(status) == -1 ) {
                return NULL;
            }
            if( len == 0 ) {
                return NULL;
